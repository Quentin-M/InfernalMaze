...
public class MazeLevel {
	...
	i = 0;
	xWidth = 0;
	for (i = 0; i < this.getMazes().length; i++) { // N�c�ssitent un retour � la ligne
	    xWidth = 0;
	    for (j = 0; j < this.getMazes()[i].length; j++) { // Deux j pour le m�me i sont sur la m�me ligne
		Maze theMaze = this.getMazes()[i][j];
		for (k = 0; k < theMaze.getHeight(); k++) { // Hauteur
		    for (l = 0; l < theMaze.getWidth(); l++) { // Largeur
			/*
			 * System.out.println(Level); System.out.println(this.getMazes()[i][j]); System.out.print(temp+" Width : "+this.getMazes()[i][j].getWidth()+" Height : "+this.getMazes()[i][j].getHeight()); System.out.println(" i :"+i+" j :"+j+" l :"+l+" k:"+k);
			 */
			// temp++;
	...
	for (i = 0; i < this.getMazes().length; i++) { // N�c�ssitent un retour � la ligne
		...
		if (i == 0) {
		    for (k = 0; k < theMaze.getHeight(); k++) { // Hauteur
			if (j == 0) { // on prend la premi�re colonne
			    for (l = 0; l < theMaze.getWidth(); l++) { // Largeur
				Level.setGrid(l, k, theMaze.getCell(l, k));
			    }
			} else {
