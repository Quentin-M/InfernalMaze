...
public class Graph {
	...
	for (int i = 0; i < this.getNodes().size(); i++) {
	    if (this.getNode(i).getLevel() == level)
		NodeByLevel.add(this.getNode(i));
	}
	return NodeByLevel;
    }
     ...
     * 
     * @param nodes
     *            ArrayList<> the list of node to set.
     */
    ...
    /**
     * Create a node and add it to the graph with and Id but nothing about the level in the tower. This function should not be used anymore
     */
    ...
    /**
     * return the number of current Node in the graph (The last node Id is this number less one)
     * 
    ...
    /**
     * create a node but doesn't add it to the graph it's return instead. Use with addNode(Node N) to link it to the node list of the graph
     * 
     * @return
     */
